      ...
      <View style={styles.duetimeComponentBlock}>
        ...
        >
          ...
          {!loading && notifications.length === 0 && (
            <Text style={styles.emptyText}>No news notifications</Text>
          )}
          {!loading && notifications.length > 0 && (
              ...
              {notifications.map((notif: any) => (
                <View key={notif.id} style={styles.news}>
                  {notif.notification_image_url ? (
                    <Image source={{ uri: notif.notification_image_url }} style={styles.newsImg} />
                  ) : (
                    <Image source={require("../../assets/peshraft-library/home/event.jpg")} style={styles.newsImg} />
                  )}
                  <View style={styles.newsTitleAndDescriptionBlock}>
                    <Text style={styles.newsTitle}>{notif.title}</Text>
                    <Text style={styles.newsDescription}>{notif.description}</Text>
                    <Text style={styles.newsTime}>{notif.time || notif.date || ""}</Text>
                  </View>
                </View>
              ))}
            </View>
...
const styles = StyleSheet.create({
  ...
  newsImg: { width: "100%", height: 95, borderTopLeftRadius: 20, borderTopRightRadius: 20 },
  ...
  newsTime: { fontSize: 14, fontWeight: "400", color: "#9E9E9E", textAlign: "right" },
});